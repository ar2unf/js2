...
const app = new Vue({
    ...
    methods: {
        ...
        getCart() {
            this.makeGETRequest(`${BASE_URL}/cart`).then((basket) => {
                console.log(basket);
                this.basketGoods =basket;
                this.basketGoodsTotalprice = this.totalPrice(basket);
        ...
        addCart(good) {
            this.makePOSTRequest(`${BASE_URL}/addToCart`,good).then(response => {
                if (response) {
                    this.getCart();
                    this.isBasketEmpty = this.basketGoods.length > 0 ? false : true;
        ...
        delitemCart(id) {
            this.makePOSTRequest(`${BASE_URL}/delToCart`,{id_product: id}).then(response => {
                if (response) {
                    this.getCart();
                    this.isBasketEmpty = this.basketGoods.length > 0 ? false : true;
                    this.basketGoodsTotalprice = this.totalPrice(this.basketGoods);
